...
This Infinite Scrolling Script was written by Proto.
It is being used on my DM2193 Final to add flare to my theme. 

My theme works fine without the infinite scrolling script
*/
...
if(!tAP.pp){return;}
	...
	function sendRequest(E,R,C,D,F,G,S,A){
		...
		if(typeof E=='object'){
			var J=E.onload;var O=E.onbeforsetheader}

		else{var J=E;var O=null}
	
		if(L||I||M){H.onload=function(){J(H);H.abort()}}

		else{H.onreadystatechange=function(){if(H.readyState==4){J(H);H.abort()}}}
		...
		function K(X,V){
			...
			for(var W=0;W<T.length;W++){var U=T[W].split('=');Y(U[0],U[1])}}}
	...
	function addNextPage(oj){
		if(oj.status==404){tAP.remainFlg=false;return;}
		...
		var footer=$("footer");
		footer?footer.parentNode.appendChild(footer):null;tAP.rF=true;
	}
	...
	function watch_scroll(){
		...
		if(r<d.clientHeight*2&&tAP.rF){tAP.rF=false;p++;
